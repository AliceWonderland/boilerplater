...
const db = module.exports = new Sequelize(connectionString, {
    ...
    native: false    // let Sequelize know we can use pg-native for ~30% more speed (if you have issues with pg-native feel free to take this out and work it back in later when we have time to help)
});
...
function sync(force=false, retries=0, maxRetries=5) { //use force=true to drop all tables and re-create
    return db.sync({force})
        ...
        .catch(fail => {
            // Don't auto-create if prod or if retried too many times
